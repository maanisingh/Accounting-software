...
const AllAccounts = () => {
  ...
  const handleDeleteAccount = async (type, name) => {
    try {
      ...
      if (window.confirm(`Are you sure you want to delete the account "${name}"?`)) {
        ...
        await axiosInstance.delete(`/api/v1/account/${row.id}`);
  ...
  const handleSaveEditedAccount = async (updatedAccount) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`/api/v1/account/${selectedAccount.id}`, payload);
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.delete(`/api/v1/account/${selectedAccount.id}`);
