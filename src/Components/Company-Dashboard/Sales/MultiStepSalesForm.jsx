...
const MultiStepSalesForm = ({ onSubmit, initialData, initialStep }) => {
  ...
  const handleSaveNext = async () => {
    ...
    try {
      ...
      // Call onSubmit for backwards compatibility (localStorage)
      if (onSubmit) {
      ...
      if (key === "quotation") {
        // Quotation already has backend integration in QuotationTab
        // Just proceed to next tab
        setKey((prev) => {
        ...
        // Validate
        const errors = validateSalesOrder(formData);
        if (errors.length > 0) {
        ...
        // Save to backend
        const savedOrder = await saveSalesOrder(formData, documentIds, setDocumentIds);
        ...
        setFormData((prevData) => ({
          ...
          salesOrder: {
            ...prevData.salesOrder,
            id: savedOrder.id,
            orderNo: savedOrder.orderNumber
          },
          deliveryChallan: {
            ...prevData.deliveryChallan,
            salesOrderNo: savedOrder.orderNumber,
        ...
        // Save to backend
        const savedChallan = await saveDeliveryChallan(formData, documentIds, setDocumentIds);
        ...
        setFormData((prevData) => ({
          ...
          deliveryChallan: {
            ...prevData.deliveryChallan,
            id: savedChallan.id,
            challanNo: savedChallan.challanNumber
          },
          invoice: {
            ...prevData.invoice,
        ...
        // Save to backend
        const savedInvoice = await saveInvoice(formData, documentIds, setDocumentIds);
        ...
        setFormData((prevData) => ({
          ...
          invoice: {
            ...prevData.invoice,
            id: savedInvoice.id,
            invoiceNo: savedInvoice.invoiceNumber || prevData.invoice.invoiceNo
          },
          payment: {
            ...prevData.payment,
        ...
        // Save to backend
        const savedPayment = await savePayment(formData, documentIds, setDocumentIds);

        // Success - show completion message
        toast.success('Sales workflow completed successfully!');

        // Optionally reset form or navigate away
        // navigate('/company/salesreport');
      }

    } catch (error) {
      console.error('Save error:', error);
      // Error already shown in helper functions
    } finally {
      setSaving(false);
    }
  };
